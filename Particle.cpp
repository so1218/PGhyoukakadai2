...
Particle::Particle() {
    ...
    gravity_ = 0.23f;
    emitterRange_ = { 20, 20 };
    easeInEase_ = std::make_unique<Easing>();
    isEmit_ = false;
    camera_ = std::make_unique<Camera>();
}

void ParticleEmitter::Emitter(bool isEmit, int amount, int type)
{
    particles.reserve(amount);
    switch (type)
    {
    case ParticleType::CenterToAround:
        for (int i = 0; i < amount; ++i)
        {
            particles.push_back(Particle());
            particles[i].InitParticle0();
            particles[i].isEmit_ = isEmit;
        }
        break;
...
void ParticleEmitter::Update(Vector2* startPos, int type)
{
    switch (type)
    {
    case ParticleType::CenterToAround:
        ...
        {
            if (particles[i].isEmit_)
            {
                particles[i].UpdateParticle0(startPos);
            }
        }
        break;
...
void ParticleEmitter::Draw(int type)
{
    switch (type)
    {
    case ParticleType::CenterToAround:
...
void Particle::UpdateParticle0(Vector2* startPos)
{
    if (frameCount_ <= 0) 
    {
    ...
    if (frameCount_ == appearInterval_) 
    {
        if (!hasExisted_) 
        {
            if (!isExist_)
            {
                theta_ = static_cast<float>(rand()) / RAND_MAX * 2.0f * float(M_PI);
                rect_.wPos.x = static_cast<float>(startPos->x);
                rect_.wPos.y = static_cast<float>(startPos->y);
                vel_.x = speed_ * cosf(theta_);
                vel_.y = speed_ * sinf(theta_);
                isExist_ = true;
                easeInEase_->isEase = true;
                hasExisted_ = true;
            }
        }
    }
    ...
    if (isExist_)
    {
        ...
        if (!easeInEase_->isEase)
        {
            isExist_ = false;
            /*isEmit_ = false;*/
        }
        easeInEase_->CountEaseLinear(startColor_, endColor_, color_);
    }
    CalcVertexRectangle(&rect_);
}
void Particle::DrawParticle0()
{
    if (isExist_) {
         BlendModeFunc::UpdateBlendMode(BlendModeType::Add);
         DrawShapes::QuadWithData(&rect_, srcW_, srcH_, GH_, spriteNum_, 0, color_);
         Novice::DrawEllipse((int)rect_.wPos.x, (int)rect_.wPos.y, 20, 20, 0.0f, color_, kFillModeSolid);
         BlendModeFunc::ResetBlendMode();
    }
    Novice::ScreenPrintf(10, 10, "%d", isEmit_);
}
void Particle::InitParticle0()
{
    appearInterval_ = 4;
    emitterRange_ = { 20, 20 };
    easeInEase_->interval = 0.01f;
    width_ = 20;
    height_ = 20;
    amount_ = 50;
    color_ = 0xffa500ff;
    startColor_ = 0xffa500ff;
    endColor_ = 0xffa50000;
    GH_ = 0;
    srcW_ = 0;
    srcH_ =  0;
    isExist_ = true;
    ...
    isEmit_ = false;
    speed_ = 1.0f;
    easeInEase_->SetEasing(EasingType::EaseOutCirc);
}

// 上に向かって周囲にEmitterRangeの範囲で発生するパーティクル
//void Particle::UpdateParticle1(Vector2* startPos)
//{
//
//    if (frameCount_ <= 0) {
//        frameCount_ = appearInterval_;
...
//        if (!isExist_) {
//
//            ro_.wPos.x = static_cast<float>(startPos->x) + (static_cast<int>(rand()) % (int)emitterRange_.x - (int)emitterRange_.x / 2);
//            ro_.wPos.y = static_cast<float>(startPos->y) + (static_cast<int>(rand()) % (int)emitterRange_.y - (int)emitterRange_.y / 2);
//            staPos_.y = ro_.wPos.y;
//            isExist_ = true;
//            easeInEase_->isEase = true;
//            blendMode.mode = blendMode.choiceType;
//
//        }
//       
//    }
...
//}
//
//void Particle::DrawParticle1()
//{
//    if (isExist_) {
//        BlendModeFunc::UpdateBlendMode();
//
//        DrawShapes::QuadWithData(&particle, srcW_, srcH_, GH_, spriteNum_, 0, color_);
//
//        BlendModeFunc::ResetBlendMode();
//    }
//    
...
//    height_ = 20;
//    color_ = 0xffff00ff;
//    startColor_ = 0xffff00ff;
//    endColor_ = 0xffff0000;
//    GH_ = 0;
//    srcW_ = 80;
//    srcH_ = 80;
//    isExist_ = false;
