...
public:
    ...
    // カメラ行列の作成
    void MakeCameraMatrix(RectangleObject* rect);
    void MakeCameraMatrix(Vector2& pos, Vector2& scale, float& theta, Vector2& translate_);

    // 矩形のカメラのデバッグ用関数
    void DebugCameraMovement();

    // 矩形の拡縮、回転
    void DebugRectMovement(RectangleObject* ro);
